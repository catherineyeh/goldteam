...
if  DEBUG && (DEBUG[0].eql? debugflag)
  code = `cd kern/conf; ./config ASST0 > /dev/null`
  puts "Just ran config"
  code = `cd kern/compile/ASST0; make depend && make && make install > /dev/null`
  ...
  code = `cd kern/conf; ./config ASST0 > /dev/null 2> /dev/null`
  puts "Just ran config"
  code = `cd kern/compile/ASST0; make depend && make && make install > /dev/null 2> /dev/null`
  puts "Just ran make depend, make and make install"
